...
@WebAppConfiguration
public class APIGeneralCheckupTest {
    ...
    public void testGetNonExistentOfficeVisit () throws Exception {
        mvc.perform( get( "/api/v1/generalcheckups/-1" ) ).andExpect( status().isNotFound() );
    }
    ...
    public void testDeleteNonExistentOfficeVisit () throws Exception {
        mvc.perform( delete( "/api/v1/generalcheckups/-1" ) ).andExpect( status().isNotFound() );
    }
    ...
    public void testPreScheduledGeneralCheckup () throws Exception {
        ...
        appointmentForm.setDate( "2030-11-19T04:50:00.000-05:00" ); // 2030-11-19 4:50 AM
    ...
    public void testGeneralCheckupAPI () throws Exception {
        ...
        OfficeVisit v = new GeneralCheckup( visit );
        ...
        mvc.perform( get( "/api/v1/generalcheckups/" + id ) ).andExpect( status().isOk() )
                .andExpect( content().contentType( MediaType.APPLICATION_JSON_UTF8_VALUE ) );
                
        visit.setDate( "2048-04-16T09:45:00.000-04:00" ); // 4/16/2048 9:45 AM

        mvc.perform( put( "/api/v1/generalcheckups/" + id ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isOk() )
                .andExpect( content().contentType( MediaType.APPLICATION_JSON_UTF8_VALUE ) );

        // PUT with the non-matching IDs should fail
        mvc.perform( put( "/api/v1/generalcheckups/" + 1 ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isConflict() );

        // PUT with ID not in database should fail
        final long tempId = 101;
        visit.setId( "101" );
        mvc.perform( put( "/api/v1/generalcheckups/" + tempId ).contentType( MediaType.APPLICATION_JSON )
